...
PandemoniumBoard::PandemoniumBoard()
    : Board(new Zobrist()),
    m_kingSquare{0,0},
    m_promotionRank(0),
    m_minIndex(0),
    ...
    m_history()
{
    ...
    setPieceGraphics(Pawn, "PANDP");
    setPieceType(PromotedPawn, tr("promoted-pawn"), "+P", FerzMovement|WazirMovement, "PANDPP");
    setPieceType(Rook, tr("rook"), "R", RookMovement, "PANDR");
    setPieceType(PromotedRook, tr("p-rook"), "+R", RookMovement|FerzMovement, "PANDPR");
    setPieceType(Bishop, tr("bishop"), "B", BishopMovement, "PANDPB");
    setPieceType(PromotedBishop, tr("promoted-bishop"), "+B", BishopMovement|WazirMovement, "PANDPB");
    setPieceType(Knight, tr("knight"), "N", KnightMovement, "PANDN");
    setPieceType(PromotedKnight, tr("promoted knight"), "+N", KnightMovement|AlfilMovement, "PANDPN");
    setPieceType(Erlking, tr("erlking"), "E", FerzMovement|WazirMovement, "PANDE");
    setPieceType(PromotedErlking, tr("p-erlking"), "+E", FerzMovement|WazirMovement|DabbaMovement, "PANDPE");
    setPieceType(Ferz, tr("ferz"), "F", FerzMovement, "PANDF");
    setPieceType(PromotedFerz, tr("p-ferz"), "+F", RookMovement|BishopMovement, "PANDPF");
    setPieceType(Van, tr("van"), "V", WazirMovement, "PANDV");
    setPieceType(PromotedVan, tr("p-van"), "+V", RookMovement|KnightMovement, "PANDPV");
    setPieceType(Alfil, tr("alfil"), "A", AlfilMovement, "PANDA");
    setPieceType(PromotedAlfil, tr("p-alfil"), "+A", BishopMovement|KnightMovement, "PANDPA");
    setPieceType(Bull, tr("bull"), "U", DabbaMovement, "PANDU");
    setPieceType(PromotedBull, tr("p-bull"), "+U", FerzMovement|WazirMovement|KnightMovement, "PANDPU");
    setPieceType(King, tr("king"),"K");
    setPieceGraphics(King, "PANDK");
}

Board* PandemoniumBoard::copy() const
{
    return new PandemoniumBoard(*this);
}

QList<Piece> PandemoniumBoard::reservePieceTypes() const
{
    QList<Piece> list;
...
QString PandemoniumBoard::variant() const
{
    return "pandemonium";
}

bool PandemoniumBoard::variantHasDrops() const
{
    return true;
}

bool PandemoniumBoard::variantHasOptionalPromotions() const
{
    return true;
}

QString PandemoniumBoard::defaultFenString() const
{
    return "rnbekebnr/2+a1+u1+f2/p1p1p1p1p/4v4/9/4V4/P1P1P1P1P/2+F1+U1+A2/RNBEKEBNR[-] w - 1";
}

int PandemoniumBoard::width() const
{
    return 9;
}

Board::CoordinateSystem PandemoniumBoard::coordinateSystem() const
{
    return Chess::Board::NormalCoordinates;
}

int PandemoniumBoard::height() const
{
    return 9;
}

int PandemoniumBoard::reserveType(int pieceType) const
{
    return normalPieceType(pieceType);
}

int PandemoniumBoard::normalPieceType(int type) const
{
    switch (type)
    {
    case PromotedPawn:
...
int PandemoniumBoard::promotedPieceType(int type) const
{
    switch (type)
    {
    case Pawn:
    ...
    case Ferz:
        return PromotedFerz;
    case Alfil:
        return PromotedAlfil;
    case Knight:
        return PromotedKnight;
    case Erlking:
        return PromotedErlking;
    case Rook:
        return PromotedRook;
...
int PandemoniumBoard::promotionRank() const
{
    // Promotion zone is opponent's third.
    return height() - height() / 3;
}

bool PandemoniumBoard::hasImpassePointRule() const
{
    return false;
}
...
void PandemoniumBoard::vInitialize()
{
    ...
    m_alfilOffsets[0] = 2 * (-arwidth - 1);
    m_alfilOffsets[1] = 2 * (-arwidth + 1);
    m_alfilOffsets[2] = 2 * (arwidth - 1);
    m_alfilOffsets[3] = 2 * (arwidth + 1);
...
QString PandemoniumBoard::vFenString(Board::FenNotation) const
{
    QString fen;
...
bool PandemoniumBoard::vSetFenString(const QStringList& fen)
{
    if (fen.size() < 2)
        return false;
...
QString PandemoniumBoard::lanMoveString(const Move& move)
{
...
QString PandemoniumBoard::sanMoveString(const Move& move)
{
    QString str;
    int source = move.sourceSquare();
    int target = move.targetSquare();
...
Move PandemoniumBoard::moveFromLanString(const QString& istr)
{
    QString str(istr);
...
Move PandemoniumBoard::moveFromSanString(const QString& str)
{
    if (str.length() < 2)
        return Move();
...
void PandemoniumBoard::vMakeMove(const Move& move, BoardTransition* transition)
{
    Side side = sideToMove();
    int source = move.sourceSquare();
    int target = move.targetSquare();
...
void PandemoniumBoard::vUndoMove(const Move& move)
{
    int source = move.sourceSquare();
    int target = move.targetSquare();
    int prom = move.promotion();
...
*/
bool PandemoniumBoard::rankIsAllowed(int, int) const
{
    return true;
}
 ...
 */
bool PandemoniumBoard::fileIsAllowed(int, int) const
{
    return true;
}

bool PandemoniumBoard::inPromotionZone(int square) const
{
...
void PandemoniumBoard::generateMovesForPiece(QVarLengthArray<Move>& moves,
                        int pieceType,
                        int square) const
{
    // Generate drops
    ...
    if (pieceHasMovement(pieceType, square, DabbaMovement))
        generateHoppingMoves(square, m_dabbaOffsets, moves1);
    if (pieceHasMovement(pieceType, square, AlfilMovement))
        generateHoppingMoves(square, m_alfilOffsets, moves1);
    if (pieceHasMovement(pieceType, square, KnightMovement))
        generateHoppingMoves(square, m_knightOffsets, moves1);
...
bool PandemoniumBoard::ranksAreAllowed() const
{
    return true;
}

bool PandemoniumBoard::isLegalPosition()
{
...
bool PandemoniumBoard::vIsLegalMove(const Move& move)
{
...
bool PandemoniumBoard::inCheck(Side side, int square) const
{
    if (square == 0)
...
int PandemoniumBoard::kingSquare(Side side) const
{
...
Result PandemoniumBoard::impassePointRule(int points, int pieces) const
{
    // 27-Point rule
    Side side = sideToMove();
    if (side == Side::Black)
...
Result PandemoniumBoard::resultFromImpassePointRule() const
{
    Side side = sideToMove();
...
Result PandemoniumBoard::result()
{
    Side side = sideToMove();
    QString str;
